...
import { calculateFdCalculator } from '@calculators/savings/fd-calculator';
import { FD_CONFIG } from '@calculators/configs/fd-calculator.config';
...
export default function FdFlagshipWidget({ initialValues = {} }) {
  const defaultState = {
    amount: 100000,
    rate: FD_CONFIG.defaultRate,
    tenure: FD_CONFIG.defaultTenureYears,
    tenureType: 'years',
  ...
  const results = useMemo(() => {
    return calculateFdCalculator(state);
  }, [state]);

  const presets = FD_CONFIG.presets;
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/20 text-emerald-300 text-xs font-semibold rounded-full border border-emerald-500/30">
              💰 Institutional Fixed Deposit Decision Engine
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
              {results.heroText}
            </h2>
            <p className="text-slate-300 text-sm sm:text-base max-w-2xl">
              Model quarterly bank compounding returns, monthly or quarterly interest income payouts, senior citizen rate bonuses (+0.50%), Section 194A TDS tax deductions, and post-tax net yield.
            </p>
          </div>
          <div className="bg-blue-900/50 border border-blue-500/40 p-4 rounded-xl text-center min-w-[220px]">
            <span className="text-xs uppercase tracking-wider text-blue-300 font-bold block">
              {state.payoutType === 'cumulative' ? 'Guaranteed Maturity Value' : 'Periodic Interest Income'}
            </span>
            <span className="text-3xl font-black text-emerald-400 mt-1 block">
              {formatCurrency(
                state.payoutType === 'cumulative' ? results.maturityValue : results.periodicPayout,
                state.currency
              )}
              {state.payoutType === 'cumulative' ? '' : `/${state.payoutType === 'monthly' ? 'mo' : 'qtr'}`}
            </span>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-2xl shadow-md border border-slate-200 dark:border-slate-700 space-y-6">
        <div className="flex flex-wrap items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-3 gap-4">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">
            1. Payout Mode & Senior Citizen Selection
          </h3>
        ...
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-700 max-w-xl">
          <button
            type="button"
            onClick={() => updateState('payoutType', 'cumulative')}
            className={`flex-1 py-2 px-3 rounded-xl text-xs sm:text-sm font-bold transition-all ${
              state.payoutType === 'cumulative'
          ...
          <button
            type="button"
            onClick={() => updateState('payoutType', 'monthly')}
            className={`flex-1 py-2 px-3 rounded-xl text-xs sm:text-sm font-bold transition-all ${
              state.payoutType === 'monthly'
                ? 'bg-blue-600 text-white shadow-md'
                : 'text-slate-600 dark:text-slate-400'
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => updateState('payoutType', 'quarterly')}
            className={`flex-1 py-2 px-3 rounded-xl text-xs sm:text-sm font-bold transition-all ${
              state.payoutType === 'quarterly'
                ? 'bg-blue-600 text-white shadow-md'
                : 'text-slate-600 dark:text-slate-400'
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <FormInputNumber
            id="amount"
            label={`Deposit Principal Amount (${currencySymbol})`}
            value={state.amount}
            min={1000}
            max={10000000}
            step={5000}
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-slate-700 space-y-4">
          <h4 className="text-sm font-bold text-indigo-600 dark:text-indigo-400">
            2. Tax & Section 194A TDS Settings
          </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 text-xs">
              <span className="font-semibold text-slate-600 dark:text-slate-300">Sec 194A TDS Cap:</span>
              <span className="font-bold text-slate-900 dark:text-white">
                {state.currency === 'INR' ? (state.isSeniorCitizen ? '₹50,000 / yr' : '₹40,000 / yr') : 'N/A'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
          ...
          <span className="text-xs text-slate-500 block">Gross maturity outgo</span>
        </div>

        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Total Interest Earned</span>
          <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400 block">
            {formatCurrency(results.totalInterest, state.currency)}
          </span>
          <span className="text-xs text-emerald-600 font-bold block">
            ({Math.round((results.totalInterest / results.principal) * 100)}% return)
          </span>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Net Post-Tax Yield</span>
          <span className="text-2xl font-black text-indigo-600 dark:text-indigo-400 block">
            {results.netEffectiveYield}% p.a.
          </span>
          <span className="text-xs text-slate-500 block">
            Post-{state.marginalTaxRate}% slab tax
          </span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
        <div className="flex justify-between items-center text-sm font-bold">
          <span className="text-blue-600 dark:text-blue-400">
            Principal Deposit: {formatCurrency(results.principal, state.currency)} ({Math.round((results.principal / (results.maturityValue || 1)) * 100)}%)
          </span>
        ...
        <div className="h-4 w-full bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden flex">
          <div
            ...
            style={{ width: `${(results.principal / (results.maturityValue || 1)) * 100}%` }}
          />
          <div
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-700 pb-3">
          📈 Fixed Deposit Accumulation Schedule
        </h3>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {results.yearlyRows.map((row) => (
                <tr key={row.year} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="p-3">{formatCurrency(row.principal, state.currency)}</td>
      ...
      {/* 7. Scenario Comparison Grid */}
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-700 pb-3">
          📊 Fixed Deposit Scenario Matrix Comparison
        </h3>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {results.scenarios.map((sc) => (
                <tr key={sc.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="p-3 text-emerald-600 dark:text-emerald-400 font-medium">{formatCurrency(sc.totalInterest, state.currency)}</td>
      ...
      {/* 8. Share & Reset Bar */}
      <ShareActions title="Fixed Deposit (FD) Calculator" />
    </div>
  );
}
